    ...
    private static function roster_table($r, $title)
    {
        ...
        switch ($title) {
            case VATROC::$ATC_LOCAL:
                $ret .= "<h1>ATC Roster</h1>";
                break;
            case VATROC::$ATC_TIMELINE:
                $ret .= "<h1>ATC Roster Timeline</h1>";
                break;
            case VATROC::$ATC_VISITING:
                $ret .= "<h1>Visiting Controller</h1>";
                break;
            case VATROC::$ATC_SOLO:
                $ret .= "<h1>Solo OJT Validation</h1>";
                break;
        }
        ...
        switch ($title) {
            case VATROC::$ATC_VISITING:
                $ret .= "<th>HOME DIVISION</th>";
                break;
            case VATROC::$ATC_SOLO:
                $ret .= "<th>SOLO VALID UNTIL</th>";
                break;
            case VATROC::$ATC_TIMELINE:
        ...
        foreach ($r as $idx => $atc) {
            ...
            switch ($title) {
                case VATROC::$ATC_VISITING:
                    $ret .= "<td>{$atc["vatroc_home_division"]}</td>";
                    break;
                case VATROC::$ATC_SOLO:
                    $ret .= "<td>{$atc["vatroc_solo_valid_until"]}</td>";
                    break;
                case VATROC::$ATC_TIMELINE:
                    if (current_user_can(VATROC::$ins_options)) {
        ...
        switch ($type) {
            case VATROC::$ATC_LOCAL:
                foreach ($data as $idx => $val) {
                    if (
                        isset($data[$idx]["{$meta_prefix}position"]) &&
